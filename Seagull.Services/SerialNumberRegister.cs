...
namespace Seagull.Services
{
    public class SerialNumberRegister
    {
            ...
            if (realCode != "")
            {
                if (currentCode.TrimEnd().Equals(realCode.TrimEnd()))
                {
                    Microsoft.Win32.RegistryKey retkey =
                         ...
                         OpenSubKey("software", true).CreateSubKey("Seagull.BeautifyFace").
                         CreateSubKey("Seagull.BeautifyFace.ini").
                         CreateSubKey(currentCode.TrimEnd());
                    retkey.SetValue("Seagull.BeautifyFace", "BBC6D58D0953F027760A046D58D52786");
        ...
        public bool BoolRegist(string sn)
        {
            ...
            try
            {
                keynames = localRegKey.OpenSubKey("software\\Seagull.BeautifyFace\\Seagull.BeautifyFace.ini\\" + GetMd5(sn)).GetValueNames();
                foreach (string name in keynames)
                {
                    if (name == "Seagull.BeautifyFace")
                    {
                        if (localRegKey.OpenSubKey("software\\Seagull.BeautifyFace\\Seagull.BeautifyFace.ini\\" + GetMd5(sn)).GetValue("Seagull.BeautifyFace").ToString() == "BBC6D58D0953F027760A046D58D52786")
                            flag = true;
                    }
                }
                keynames = userRegKey.OpenSubKey("software\\Seagull.BeautifyFace\\Seagull.BeautifyFace.ini\\" + GetMd5(sn)).GetValueNames();
                foreach (string name in keynames)
                {
                    if (name == "Seagull.BeautifyFace")
                    {
                        if (flag && userRegKey.OpenSubKey("software\\Seagull.BeautifyFace\\Seagull.BeautifyFace.ini\\" + GetMd5(sn)).GetValue("Seagull.BeautifyFace").ToString() == "BBC6D58D0953F027760A046D58D52786")
                            return true;
                    }
                }
                return false;
